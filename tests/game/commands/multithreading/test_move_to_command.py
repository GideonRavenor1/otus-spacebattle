...
def test_move_to_with_valid_params() -> None:
    """
    В данном тесты мы проверяем, что отработает первая макрокоманда "forward", остальные передаются в другую очередь и
    ...
    command_container.resolve("command.hard_stop", params=params)
    ...
    commands = [forward_command, move_to_command, forward_rotate_command]
